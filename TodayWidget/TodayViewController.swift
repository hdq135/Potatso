...
class TodayViewController: UIViewController, NCWidgetProviding, UITableViewDataSource, UITableViewDelegate, GCDAsyncSocketDelegate {
    ...
    override init(nibName nibNameOrNil: String?, bundle nibBundleOrNil: Bundle?) {
        ...
        socket = GCDAsyncSocket(delegate: self, delegateQueue: DispatchQueue.global(qos: DispatchQoS.background.qosClass))
    }
    ...
    func updateStatus(current: Bool) {
        if status != current {
            ...
            DispatchQueue.main.async {
                self.tableView.reloadRows(at: [NSIndexPath(row: 0, section: 0) as IndexPath], with: .none)
            }
        }
    }
    ...
    func switchVPN() {
        if status {
            ...
            URLSession.shared.dataTask(with: NSURL(string: "https://on-demand.connect.potatso.com/start/")! as URL).resume()
        }
    }
